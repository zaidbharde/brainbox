...
import { Home, MonitorCog, Play, RotateCcw } from 'lucide-react';
...
const turboStarterCode = `#include <graphics.h>
#include <conio.h>
#include <dos.h>
...
int main() {
  ...
  setcolor(WHITE);
  rectangle(80, 120, 280, 300);
  setfillstyle(SOLID_FILL, LIGHTCYAN);
  floodfill(120, 200, WHITE);

  outtextxy(305, 210, "Turbo C++ graphics + animation working");
  getch();
  closegraph();
  return 0;
}`;
...
function clearCanvas(canvas, fillColor = '#000000') {
  if (!canvas) {
    return;
  }
...
async function executeGraphicsCall(functionName, rawArgs, exec) {
  ...
  if (fn === 'initgraph') {
    clearCanvas(canvas, state.background);
    shapes.length = 0;
    exec.commandCount += 1;
    return;
  }
  ...
  if (fn !== 'main') {
    exec.warnings.push(`Unsupported Turbo call skipped: ${functionName}()`);
  }
}
...
async function renderGraphicsProgram(code, canvas, shouldStop) {
  if (!canvas) {
  ...
  clearCanvas(canvas, exec.state.background);
  ...
  return {
    ok: true,
    message: `[ok] Turbo graphics rendered. Commands executed: ${exec.commandCount}${stdoutText}${warningText}`,
  };
}

export default function TurboC() {
  const [code, setCode] = useState(turboStarterCode);
  const [output, setOutput] = useState('Ready. Write Turbo C++ code and click Run C++.');
  ...
  useEffect(() => {
    clearCanvas(screenCanvasRef.current, '#000000');
    return () => {
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      ...
      setRunMode('cpp');
      setOutput(stripAnsi(composed) || '[ok] Program ran successfully with no output.');
      clearCanvas(screenCanvasRef.current, '#000000');
    } catch (error) {
      if (runTokenRef.current !== token) {
        return;
  ...
  const resetWorkspace = useCallback(() => {
    runTokenRef.current += 1;
    setRunning(false);
    setCode(turboStarterCode);
    setOutput('Reset complete. Turbo starter restored.');
    setRunMode('idle');
    clearCanvas(screenCanvasRef.current, '#000000');
  }, []);
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#0f172a] text-slate-100">
      <div className="mx-auto flex h-full w-full max-w-[1750px] flex-col px-4 py-4 sm:px-6">
        <header className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-cyan-200/20 bg-white/5 px-4 py-3 shadow-[0_16px_40px_rgba(0,0,0,0.45)] backdrop-blur">
          <div className="flex items-center gap-3">
            <MonitorCog className="h-5 w-5 text-cyan-200" />
            <h1 className="text-lg font-semibold text-white">Turbo C++ Lab</h1>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              onClick={runCode}
              disabled={running}
              className="inline-flex items-center gap-2 rounded-md border border-cyan-300/55 bg-cyan-300/15 px-3 py-2 text-sm font-medium text-cyan-100 transition hover:bg-cyan-300/25 disabled:opacity-60"
            >
            ...
            <Link
              to="/"
              className="inline-flex items-center gap-2 rounded-md border border-slate-500/55 bg-slate-700/25 px-3 py-2 text-sm text-slate-100 transition hover:border-slate-300/70"
            >
        ...
        <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[1.05fr_0.95fr]">
          <section className="min-h-0 overflow-hidden rounded-2xl border border-cyan-200/20 bg-white/5 shadow-[0_18px_45px_rgba(0,0,0,0.42)] backdrop-blur">
            <div className="border-b border-cyan-200/15 bg-black/20 px-4 py-2 text-sm text-slate-300">Turbo C++ Code Editor</div>
            <div className="h-[calc(100%-41px)] min-h-[360px]">
              <Editor
                height="100%"
                defaultLanguage="cpp"
                value={code}
          ...
          <section className="min-h-0 overflow-hidden rounded-2xl border border-cyan-200/20 bg-black shadow-[0_24px_55px_rgba(0,0,0,0.55)]">
            <div className="border-b border-cyan-200/20 bg-[#020816] px-4 py-2 text-sm text-cyan-100">
              Turbo Screen
            </div>
            <div className="flex h-[calc(100%-41px)] flex-col">
              <div className="flex-1 items-center justify-center p-3">
                <canvas
                  ref={screenCanvasRef}
                  width={CANVAS_WIDTH}
                  height={CANVAS_HEIGHT}
                  className="h-full w-full rounded border border-cyan-200/20 bg-black"
                />
              </div>

              <div className="border-t border-cyan-200/20 bg-[#020816] px-4 py-3 font-mono text-sm text-slate-100">
                <p className="mb-2 text-xs uppercase tracking-wide text-cyan-200/85">
                  Output {runMode === 'graphics' ? '(Graphics Mode)' : runMode === 'cpp' ? '(C++ Mode)' : ''}
                </p>
                <pre className="max-h-[140px] overflow-auto whitespace-pre-wrap break-words">{output}</pre>
              </div>
            </div>
          </section>
        </div>
